...
// vars for spiral
float arcLength;
// vars for squarebloom (UNUSED SCENE)
float xoffset = 0.0;
float yrise = 0.0;
int col;
color[] colorarray;
...
void setup() {
  //size(1280, 720, P3D); //use windowed mode if fullscreen is buggy
  fullScreen(P3D);
  frameRate(60);
  background(0);
  noCursor();
  ...
  player = minim.loadFile("koan.mp3", 1024); // koan sound - meanwhile in the future (demonstrates more energy)
  //player = minim.loadFile("drowning.mp3", 1024); // pyrmdplaza - drowning (demonstrates more fluidity)
  //player = minim.loadFile("cena.mp3", 1024); // JOOOOOOHN CENAAAAA (yes, it works too)

  // FFT object with time domain buffer
  fft = new FFT(player.bufferSize(), player.sampleRate());

  // setup for scene selector
  selector = 2; // the scene to select on frame 1
  weightage = 1;
  ...
  // (UNUSED SCENE) setup for squarebloom
...
void draw() {
  ...
  noiseAmp = map(fft.getFreq(19000), 0, 1, 0, 500); // analyse amplitude of 19kHz and scale it up

  // changing of variables according to sound analysis
  if (bassAmp>kickboxSensitivity) {
    selector = floor(random(1, 5.21)); //randomizes value used in scene selection
...
// squarebloom (UNUSED SCENE) -- removed it because it didn't fit well with other scenes I made. It's still pretty nice though.
