...
function Character() {
  ...
  this.anim = function() { //sets up the characters. will go in setup function on main sketch
    ...
    ghost = createSprite(900, (11/14)*windowHeight, 50, 100); //creates the Apple sprite
 ...
 //   myAnimation1.offY = 18;
  ...
  this.walking = function() {
    ...
    if (keyDown (LEFT_ARROW)) {
      ...
      ghost.velocity.x = - 4;//negative x velocity: move left
    } else if (keyDown(RIGHT_ARROW)) {
      ghost.changeAnimation("moving"); // move to the right
      
      ghost.mirrorX(-1);//unflip 
      ghost.velocity.x = 4; // possitive velocity 
    } else {
    ...
    if (ghost.position.x > 1100) { // if the apple is past a certain position on the screen it will delete and strawberry will win the game
      ghost.remove();
      textSize(72);
      fill(203, 13, 0);
    ...
    if (keyDown ("a")) { // repeat all the same functions as apple with strawberry but will use wasd and c to move and punch
      ...
      straw.velocity.x = - 4;  //negative x velocity: move left
    ...
    if (keyWentDown("c")) { //punching with c. same as apple punching piece
      ...
      ghost.bounce(straw, function() {
        ghost.velocity.x = 50; // pushes back the apple
      }
      );
    } else {
      straw.immovable = false;
    }
    if (straw.position.x <100) { // will remove the strawberry if passes x=100
      straw.remove();
      textSize(72);
      fill(0, 187, 39);
