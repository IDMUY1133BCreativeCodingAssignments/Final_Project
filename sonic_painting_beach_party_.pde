...
void draw() {
  ...
  if(keyCode == UP){ //press up Key to switch to second track/remix and add thereby to sketch
    currentTrack=1;  
...
// exit condition is when branch smaller than 2 pixels
 ...
 void waves(){ 
  for (int m = 0; m < player.bufferSize() - 1; m+=200) //m is new i 
  {
    ...
    endShape();
    // END WAVES 
    //ref. examples in Processing for minim, particularly for Frequency Energy Beat Detection
    }
  }
  
  void stars(){
    //STARS
    //ref. examples in minim
  ...
  void tree(){
    // TREE 
    // ref.https://processing.org/examples/tree.html
    strokeWeight(3);
    stroke(#03AA60); 
  ...
  void island(){ //drawing sandy rock island
    noStroke();
    fill(#B7B467);
